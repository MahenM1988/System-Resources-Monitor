...
while running:
    ...
    # Render header
    header_text = header_font.render("System Resource Monitor", True, (0, 255, 0))  # Red text
    ...
    for text in info_texts:
        info_render = font.render(text, True, (0, 255, 0))  # Red text
        screen.blit(info_render, (20, y_offset))
        y_offset += 30

    # Render CPU usage
    cpu_text = font.render("CPU Cores Usage:", True, (0, 255, 0))  # Red text
    screen.blit(cpu_text, (20, y_offset))
    y_offset += 30

    for i, usage in enumerate(cpu_usage_per_core):
        core_text = font.render(f"Core {i}: {usage}%", True, (0, 255, 0))  # Red text
    ...
    memory_text = font.render(f"Memory Usage: {memory}%", True, (0, 255, 0))  # Red text
    ...
    network_sent_text = font.render(f"Network Sent: {network_sent_per_sec}/s", True, (0, 255, 0))  # Red text
    network_recv_text = font.render(f"Network Received: {network_recv_per_sec}/s", True, (0, 255, 0))  # Red text
